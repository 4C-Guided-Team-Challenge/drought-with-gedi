...
def parse_args():
    ...
    parser.add_argument("--shape_path", default="data/polygons/Amazonia_drought_gradient_polygons.shp", type=str, help="path of shapefile")
    parser.add_argument("--year_range", nargs='+', type=int, default=[2019, 2023], help="the range of years [start_year, end_year) to query")
...
def gedi_query_psql(
    ...
    columns = fields+["shot_number", "lon_lowestmode", "lat_lowestmode", QUALITY_FLAG]
    # iterate over time
    processed_data = None
    count = 0
    for year in range(year_range[0], year_range[1]):
        for month in range(1, 13):
            ...
            for i in range(len(shape)):
                ...
                filters = np.logical_and(gedi_shots_gdf[QUALITY_FLAG]==1, gedi_shots_gdf["pai"]>0) if "pai" in columns else gedi_shots_gdf[QUALITY_FLAG]==1
